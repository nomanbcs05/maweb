...
function updateSummarySidebar() {
  ...
  container.innerHTML = Object.keys(cart).map(id => {
    return `<div class="sum-item">
      ...
      <b>$${(cart[id].qty * cart[id].price).toFixed(2)}</b>
    </div>`;
  }).join('');
  
  document.getElementById('sumSub').textContent = `$${totals.subtotal.toFixed(2)}`;
  document.getElementById('sumTax').textContent = `$${totals.tax.toFixed(2)}`;
  
  // Adjust delivery fee based on order type
  const delFee = orderData.type === 'delivery' ? totals.delivery : 0;
  document.getElementById('sumDel').textContent = orderData.type === 'delivery' ? `$${delFee.toFixed(2)}` : 'Free (Pickup)';
...
*Subtotal:* $${o.totals.subtotal.toFixed(2)}
*Delivery:* $${o.totals.delivery.toFixed(2)}
*Total:* $${o.totals.total.toFixed(2)}
...
`;
  ...
  const waNum = '923342826675'; // Stripped leading 0, added 92 for Pakistan
