...
function updateStats(orders) {
  ...
  document.getElementById('statSales').textContent = `Rs. ${sales.toFixed(0)}`;
  ...
  document.getElementById('statAOV').textContent = `Rs. ${aov.toFixed(0)}`;
}
  ...
  tbody.innerHTML = orders.map(o => {
    ...
    return `
      <tr>
        ...
        <td><b>Rs. ${o.totals.total.toFixed(0)}</b></td>
        <td>
...
function initChart(orders) {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const dayOrders = orders.filter(o => o.createdAt && o.createdAt.startsWith(dateString));
    const revenue = dayOrders.reduce((sum, o) => sum + (o.totals?.total || 0), 0);
